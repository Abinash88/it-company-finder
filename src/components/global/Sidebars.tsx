        ...
        <ul className="  h-full ">
          {SidebarData?.map((item: SidebarDataTypes, index) => {
            return (
              <li
                key={index}
                className={`flex px-4 py-2 rounded-md duration-300 transition-all my-3 ${
                  location === item?.link && "bg-gray-100"
                } hover:bg-gray-100 group cursor-pointer w-full items-center `}
              >
                <Link
                  className="text-gray-600 w-full h-full flex items-center space-x-3 font-semibold duration-300 transition-all text-[15px] group-hover:text-gray-600"
                  href={item?.link}
                >
