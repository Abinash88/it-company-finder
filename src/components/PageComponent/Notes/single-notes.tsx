     ...
     group justify-start px-2 items-center gap-4  rounded-md bg-card"
        >
            ...
            <Div className=" h-full  backdrop-blur-5   top-[-12px] rounded-md  left-0">
                <Div onClick={() => { setNotePopup(data) }} className="h-full mt-5 flex flex-col gap-1 ">
                    <h2 className="text-[15px] text-gray-600 font-normal">{data?.title}</h2>
            ...
            <Div className=" flex-1 flex gap-4 px-4 items-center justify-end ">
                <FaRegCopy onClick={() => { copyToClipboard(data?.description) }} className="text-gray-600 hover:bg-gray-100 text-[22px] rounded-md p-[3px]" />
