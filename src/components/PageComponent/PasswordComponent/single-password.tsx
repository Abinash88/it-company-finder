...
import { FaEye, FaRegCopy, FaRegEdit, FaRegStar } from "react-icons/fa";
    ...
    >
      ...
      <Div className=" h-full  backdrop-blur-5   top-[-12px] rounded-md  left-0">
        <Div className="h-full mt-5 flex flex-col gap-1 ">
          <h2 className="text-[14px] text-gray-600 font-normal">{data?.name}</h2>
          <h3 className="text-[11px] text-gray-600 font-light">Password Catagory : <b className="text-gray-400 "> {data?.catagory}</b> </h3>
        </Div>
      </Div>
      <Div className=" flex-1 flex gap-4 px-4 items-center justify-end ">
        <FaEye onClick={() => { }} className="text-gray-600 hover:bg-gray-100 text-[22px] rounded-md p-[3px]" />
