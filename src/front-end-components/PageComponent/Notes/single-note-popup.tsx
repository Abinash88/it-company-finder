...
import RemoveBox from '@/front-end-components/ui/remove';
import Div from '@/lib/Div';
import React from 'react';

const SingleNotePopup = ({
  notePopup,
  setNotePopup,
}: {
  notePopup: StaticNotesDataTypes;
  setNotePopup: React.Dispatch<
    React.SetStateAction<StaticNotesDataTypes | undefined>
  >;
}) => {
  return (
    <Div className=' h-full fixed flex items-center justify-center backdrop-blur-5 inset-0 rounded-md'>
      ...
      <Div className='h-[80%] relative z-100 w-[70%] bg-white rounded-md p-4 mt-5 flex flex-col gap-1 '>
        <RemoveBox
          bgStyle='hover:!bg-gray-100'
          iconStyle='text-gray-600'
          remove={() => {
            setNotePopup(undefined);
          }}
        />
        <h2 className='text-[15px] text-gray-600 font-normal'>
          {notePopup?.title}
        </h2>
        <p className='text-[13px] w-[80%] text-gray-600 font-normal'>
          {notePopup?.description.substring(0, 200)}...{' '}
        </p>
      </Div>
    </Div>
  );
