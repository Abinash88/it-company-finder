  ...
  return (
    <Div className={cn(`flex flex-col items-start ${className}`)}>
      <Image
        ...
        alt='logo'
      />
      <h3 className='text-start text-[24px] mb-2 mt-4 font-semibold text-foreground'>
        {title}
      </h3>
      <p className=' mb-3 text-[16px] text-foreground/70'>{subtitle}</p>
    </Div>
  );
};
...
export const SubmitButton = React.forwardRef<HTMLButtonElement, ButtonTypes>(
  ({ ...rest }, ref) => {
    return (
      <button
        ref={ref}
        {...rest}
        className={`px-6 py-2 md:py-3 text-[14px] mx-auto flex justify-center bg-[#075bdb]
  ...
  return (
    <p className='text-[12px] pb-4'>
      ...
      <button
        ...
        onClick={() => router.replace(`/account?type=${type}`)}
      >
        {type}
      </button>
    </p>
...
export const AuthInputBox = React.forwardRef<HTMLInputElement, AuthInput>(
    ...
    return (
      <Div className='flex flex-col flex-1 space-y-1 '>
        ...
        <input
          ...
          id={label}
          {...props}
          className={cn(`  px-4 py-2 md:py-[10px] bg-input text-foreground text-[13px] placeholder:text-[13px]
...
export const MoreLogin = () => {
  return (
    <Div className='flex gap-4 justify-center mt-2 mb-2'>
      <button
        type='button'
        className='flex text-gray-600 hover:bg-background transition-all flex-1 items-center text-[14px] justify-center rounded-md border border-gray-300 py-2 md:py-[10px]  space-x-3'
      >
