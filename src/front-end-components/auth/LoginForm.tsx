...
const LoginForm = ({
  ...
  const dataSubmit: SubmitHandler<Omit<SignupDataTypes, 'name'>> = async (
    body
  ) => {
    setLoading(true);
    try {
      const res = await fetchRequest<
        Omit<SignupDataTypes, 'name'>,
        {
          success: boolean;
          message: string;
          accessToken: string;
  ...
  return (
    <Div className='w-[80%] mx-auto h-full md:px-10 pt-3'>
      <form action='' onSubmit={handleSubmit(dataSubmit)} className='w-full'>
        <Topheader
          title='Welcome to PersonalManager'
          subtitle='Welcome back please login here...'
        />
        <MoreLogin />
        <br />
        <OrComponent />
